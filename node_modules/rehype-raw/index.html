...
<span class="title">Quick Reference</span></a><div class="menu"><a href="https://github.com/jaywcjlove/reference/blob/main/node_modules/rehype-raw\readme.md" class="" target="__blank"><svg viewBox="0 0 36 36" fill="currentColor" height="1em" width="1em"><path d="m33 6.4-3.7-3.7a1.71 1.71 0 0 0-2.36 0L23.65 6H6a2 2 0 0 0-2 2v22a2 2 0 0 0 2 2h22a2 2 0 0 0 2-2V11.76l3-3a1.67 1.67 0 0 0 0-2.36ZM18.83 20.13l-4.19.93 1-4.15 9.55-9.57 3.23 3.23ZM29.5 9.43 26.27 6.2l1.85-1.85 3.23 3.23Z"></path><path fill="none" d="M0 0h36v36H0z"></path></svg><span>编辑</span></a><button id="darkMode" type="button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="light" height="1em" width="1em">
...
</script><a href="https://github.com/jaywcjlove/reference" class="" target="__blank"><svg viewBox="0 0 16 16" fill="currentColor" height="1em" width="1em"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"></path></svg></a></div></div></nav><div class="wrap max-container"><header class="wrap-header h1wrap"><h1 id="rehype-raw"><a aria-hidden="true" tabindex="-1" href="#rehype-raw"><span class="icon icon-link"></span></a>rehype-raw</h1><div class="wrap-body">
<p><a href="https://github.com/rehypejs/rehype-raw/actions"><img src="https://github.com/rehypejs/rehype-raw/workflows/main/badge.svg" alt="Build"></a>
<a href="https://codecov.io/github/rehypejs/rehype-raw"><img src="https://img.shields.io/codecov/c/github/rehypejs/rehype-raw.svg" alt="Coverage"></a>
<a href="https://www.npmjs.com/package/rehype-raw"><img src="https://img.shields.io/npm/dm/rehype-raw.svg" alt="Downloads"></a>
<a href="https://bundlephobia.com/result?p=rehype-raw"><img src="https://img.shields.io/bundlephobia/minzip/rehype-raw.svg" alt="Size"></a>
...
<p><strong><a href="https://github.com/rehypejs/rehype">rehype</a></strong> plugin to parse the tree (and raw nodes) again, keeping
positional info okay.</p>
...
<li><a href="#api">API</a>
<ul>
<li><a href="#unifieduserehyperaw-options"><code>unified().use(rehypeRaw[, options])</code></a></li>
</ul>
</li>
...
<p>This package is a <a href="https://github.com/unifiedjs/unified">unified</a> (<a href="https://github.com/rehypejs/rehype">rehype</a>) plugin to parse a document again.
To understand how it works, requires knowledge of ASTs (specifically, <a href="https://github.com/syntax-tree/hast">hast</a>).
This plugin passes each node and embedded raw HTML through an HTML parser
...
This is a rehype plugin that parses the tree again.</p>
...
<p>This plugin is particularly useful when coming from markdown and wanting to
support HTML embedded inside that markdown (which requires passing
<code>allowDangerousHtml: true</code> to <code>remark-rehype</code>).
Markdown dictates how, say, a list item or emphasis can be parsed.
We can use that to turn the markdown syntax tree into an HTML syntax tree.
But markdown also dictates that things that look like HTML, are passed through
untouched, even when it just looks like XML but doesn’t really make sense, so we
...
(you can pass <code>allowDangerousHtml: true</code> to <code>rehype-stringify</code>, which passes
HTML through untouched).
But there are two main cases where a proper syntax tree is preferred:</p>
<ul>
<li>rehype plugins need a proper syntax tree as they operate on actual nodes to
...
<p>This plugin is built on <a href="https://github.com/syntax-tree/hast-util-raw"><code>hast-util-raw</code></a>, which does the work on
syntax trees.
rehype focusses on making it easier to transform content by abstracting such
internals away.</p>
...
<pre class="language-sh"><code class="language-sh code-highlight"><span class="code-line"><span class="token function">npm</span> <span class="token function">install</span> rehype-raw
</span></code></pre>
<p>In Deno with <a href="https://www.skypack.dev">Skypack</a>:</p>
<pre class="language-js"><code class="language-js code-highlight"><span class="code-line"><span class="token keyword module">import</span> <span class="token imports">rehypeRaw</span> <span class="token keyword module">from</span> <span class="token string">'https://cdn.skypack.dev/rehype-raw@6?dts'</span>
</span></code></pre>
...
</span></span></span><span class="code-line"><span class="token script"><span class="token language-javascript">  <span class="token keyword module">import</span> <span class="token imports">rehypeRaw</span> <span class="token keyword module">from</span> <span class="token string">'https://cdn.skypack.dev/rehype-raw@6?min'</span>
...
<pre class="language-markdown"><code class="language-markdown code-highlight"><span class="code-line"><span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>note<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
</span><span class="code-line">
</span><span class="code-line">A mix of <span class="token italic"><span class="token punctuation">*</span><span class="token content">markdown</span><span class="token punctuation">*</span></span> and <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;</span>em</span><span class="token punctuation">></span></span>HTML<span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;/</span>em</span><span class="token punctuation">></span></span>.
</span><span class="code-line">
...
</span><span class="code-line">    <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;</span>div</span> <span class="token attr-name">class</span><span class="token attr-value"><span class="token punctuation attr-equals">=</span><span class="token punctuation">"</span>note<span class="token punctuation">"</span></span><span class="token punctuation">></span></span>
</span><span class="code-line">      <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;</span>p</span><span class="token punctuation">></span></span>A mix of <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;</span>em</span><span class="token punctuation">></span></span>markdown<span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;/</span>em</span><span class="token punctuation">></span></span> and <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;</span>em</span><span class="token punctuation">></span></span>HTML<span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;/</span>em</span><span class="token punctuation">></span></span>.<span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;/</span>p</span><span class="token punctuation">></span></span>
</span><span class="code-line">    <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;/</span>div</span><span class="token punctuation">></span></span>
</span><span class="code-line">  <span class="token tag"><span class="token tag"><span class="token punctuation">&#x3C;/</span>body</span><span class="token punctuation">></span></span>
...
The default export is <code>rehypeRaw</code>.</p>
</div></div><div class="h2wrap-body"><div class="wrap"><div class="wrap-header h3wrap"><h3 id="unifieduserehyperaw-options"><a aria-hidden="true" tabindex="-1" href="#unifieduserehyperaw-options"><span class="icon icon-link"></span></a><code>unified().use(rehypeRaw[, options])</code></h3><div class="wrap-body">
<p>Parse the tree (and raw nodes) again, keeping positional info okay.</p>
...
to the documentation available in <a href="https://github.com/syntax-tree/hast-util-raw"><code>hast-util-raw</code></a>.</p>
...
<p>The <code>allowDangerousHtml</code> option in <code>remark-rehype</code> is dangerous, so defer
to that plugin on how to make it safe.
Otherwise, this plugin is safe.</p>
